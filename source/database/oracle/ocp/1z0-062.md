...
恢复区完全满后，显示的错误如下，其中*nnnnn*是所需的字节数，*mmmmm*是磁盘配额：
...
_`schema_name`_：视图所在的架构的名称。如果未提供架构名称，则默认为执行导入的用户。

_`view_name`_：要导入为表格的视图的名称。
...
在Oracle Database _12c中_，该参数支持以下值：
